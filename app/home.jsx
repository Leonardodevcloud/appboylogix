...
export default function Home() {
  ...
  useGPS(fila.find(e => ['em_coleta','em_rota'].includes(e.status))?.id || null);
  ...
  const carregar = useCallback(async () => {
    try {
      ...
      if (e.message.includes('401') || e.message.toLowerCase().includes('inválido')) {
  ...
  return (
    <View style={s.root}>
      ...
      {/* Status bar topo — fundo claro como no protótipo */}
                ...
                >
                  {busy[e.id] ? <ActivityIndicator color={C.azulP} size="small" />
                    : <Text style={[s.mBtnTxt, { color: C.azulP }]}>
                        {e.status === 'aguardando_coleta' ? 'Iniciar coleta' : 'Coleta concluída — saindo'}
                      </Text>}
                </TouchableOpacity>
              </View>
            ))}
